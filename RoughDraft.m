...
ephysFilePath = '/Volumes/Elements/Data/Voltage Imaging/VoltImg_slice_test/Ephys Data/230809/SSTCre_DIOASAP6_IC_Slice_080923_Cell4_dvTest';
...
ImgsFilePath = '/Volumes/Elements/Data/Voltage Imaging/VoltImg_slice_test/Imaging Data/080923/cell4/dvTest';
...
%% Rough Draft of new analysis
% Break vsTest_inputs into trials
nTrials = length(dvCondSequence);
samplesPerTrial = sweepDur*Fs;
framesPerTrial = sweepDur*imagingFreq;

trialsPerframeBlocks = 16; % CUSTOMIZE THIS NUMBER DEPENDING ON IMAGING PARAMS!!
framesPerBlock = ceil(framesPerTrial*trialsPerframeBlocks);

% For downsampling ephys data
downSampFactor = round(Fs/imagingFreq);

dvAllConds = cell(nConds, 1);
dvDownSampAllConds = cell(nConds, 1);
for tt = 1:nTrials
    dvAllConds{dvCondSequence(tt)} = [dvAllConds{dvCondSequence(tt)}, vsTest_inputs((1+(samplesPerTrial*(tt-1))):(samplesPerTrial+(samplesPerTrial*(tt-1))))];
    dvDownSampAllConds{dvCondSequence(tt)} = [dvDownSampAllConds{dvCondSequence(tt)}, downsample(vsTest_inputs((1+(samplesPerTrial*(tt-1))):(samplesPerTrial+(samplesPerTrial*(tt-1)))), downSampFactor)];
end

% Divide single sweep image into trial blocks
remainderFrame = framesPerTrial - floor(framesPerTrial);
1/remainderFrame = 
firstRoundBlock = 16; % break single image first into 'firstRoundBlock' of blocks
framesfirstRoundBlock = size(imageStack, 3)/firstRoundBlock;
